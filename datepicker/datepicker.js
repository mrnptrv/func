...
(function(F, y) {
  ...
  function z(a, c, b, d) {
    ...
    if (b && -1 !== b.indexOf(" ")) {
      ...
      for (h = a.__pickmeup.events, l = h.length, e = 0; e < l; ++e) c && c !== h[e][0] || b && b !== h[e][1] || d && d !== h[e][2] || h[e][0].removeEventListener(h[e][1],
        h[e][2])
  }
  ...
  function H(a) {
    if ("single" === a.mode) {
      ...
      return {
        formatted_date: D(c, a.format, a.locales[a.locale]),
        date: c
      }
    }
    c = {
  ...
  function W(a, c, b) {
    ...
    if (!(c instanceof Array) || 0 < c.length)
      if (d.date = t(c, d), "single" !== d.mode)
        for (d.date instanceof Array ? (d.date[0] = d.date[0] || t(new Date, d), "range" === d.mode && (d.date[1] = d.date[1] || t(d.date[0], d))) : (d.date = [d.date], "range" === d.mode && d.date.push(t(d.date[0], d))), c = 0; c < d.date.length; ++c) d.date[c] =
    ...
    if ("multiple" === d.mode)
      for (c = 0; c < d.date.length; ++c) d.date.indexOf(d.date[c]) !== c && (d.date.splice(c, 1), --c);
